...
# Data used for preprocessing of titles and tags
...
titles_contain.to_csv(DIR_OUT + "titles_date.csv",sep=";",index=False, header=True)
...
titles["title"].to_csv(DIR_OUT+"titles_to_preprocess.txt", sep="\n",index=False, header=False)
...
with codecs.open(os.path.join(DIR_OUT,"titles_to_preprocess.txt"),encoding="utf8") as f:
    books.append(f.read())
...
for i in range(0,int(len(books[0])/nlp.max_length)+1):
    ...
    doc_preprocessed = [token.text for token in doc if (not token.is_digit and not token.is_stop and not token.is_punct and not (token.text in my_undesired_list))]
    doc_preprocessed = [token.replace(',','') for token in doc_preprocessed]

    with open(DIR_OUT+"titles_words.csv", 'w') as file:
        #writing the header
        file.write('word\n')
        # writing the data
        for x in doc_preprocessed:
            file.write(x+'\n')
...
display(sum(positive_sent)/len(positive_sent))

negative_sent = []
[negative_sent.append(analyzer.polarity_scores(sent.text)['neg']) for sent in doc.sents]
display(sum(negative_sent)/len(negative_sent))
...
titles_preprocessed = pd.read_csv(DIR_OUT+"titles_words.csv", sep=',', encoding='latin1')
titles_preprocessed = [x.encode('utf-8','ignore').decode("utf-8") for x in titles_preprocessed['word']]

# Count occurences
#titles_preprocessed_lowercase = [word.lower() for word in titles_preprocessed]
word_freq = Counter(titles_preprocessed)
...
tags_contain.to_csv(DIR_OUT + "tags_date.csv",sep=";",index=False, header=True)
...
period_start = pd.to_datetime("2013-02-06", format='%Y-%m-%d')
period_end =pd.to_datetime("2015-02-06", format='%Y-%m-%d')
...
tags["tags"].to_csv(DIR_OUT+"tags_to_preprocess.txt", sep="\n",index=False, header=False)
...
with codecs.open(os.path.join(DIR_OUT,"tags_to_preprocess.txt"),encoding="utf8") as f:
    books.append(f.read())
...
for i in range(0,int(len(books[0])/nlp.max_length)+1):
    ...
    doc_preprocessed = [token.text for token in doc if (not token.is_digit and not token.is_stop and not token.is_punct and not (token.text in my_undesired_list))]
    doc_preprocessed = [token.replace(',','') for token in doc_preprocessed]
    doc_preprocessed = [x.encode('utf-8','ignore').decode("utf-8") for x in doc_preprocessed]
...
tags_preprocessed = pd.read_csv(DIR_OUT+"tags_words.csv", sep=',', encoding='latin1')
tags_preprocessed = [x.encode('utf-8','ignore').decode("utf-8") for x in tags_preprocessed['word']]

# Count occurences
#tags_preprocessed_lowercase = [word.lower() for word in tags_preprocessed]
word_freq = Counter(tags_preprocessed)
common_words = word_freq.most_common()
common_words_out = pd.DataFrame(common_words)
common_words_out.columns=['word','occurences']
